...
class APTOSExternalCrossValidation(BaseCrossValidation):
    def __init__(self, training_config):
        ...
        external_train_dir = self.data_setting['external_train_dir']
        external_train_path = self.data_setting['external_train_path']

        etrain_df = pd.read_csv(external_train_path)
        self.etrn_paths = [os.path.join(external_train_dir, img_name + '.jpeg') for img_name in etrain_df['image']]
        self.etrn_labels = np.array([[str(lbl)] for lbl in etrain_df['level'].tolist()])
    ...
    def __iter__(self):
        for fold_idx, (trn_idx, val_idx) in enumerate(self.sp):
            ...
            # TODO: add etrn_x, etrn_y
            trn_x = self.etrn_paths + list(trn_x)
            trn_y = np.array(list(self.etrn_labels) + list(trn_y))
